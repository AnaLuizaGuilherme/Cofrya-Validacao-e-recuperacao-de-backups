...
def salvar_arquivos_enviados(pasta_destino: Path, copia_id: str, dump, manifest, referencias) -> None:
    salvar_uploads(pasta_destino, copia_id, dump, manifest, referencias)
...
def executar_formulario(repositorio, saida, copia_id, configuracao, cenario,
    ...
    try:
        ...
        contexto = ConfiguracaoExecucao(
            chave_hmac=chave, diretorio_trabalho=str(saida.parent / "tentativas"),
...
def renderizar_pagina(repositorio: Path, saida: Path) -> None:
    """Conteúdo das abas 'Nova verificação' e 'Resultados', parametrizado
    por pasta — usado tanto pelo app standalone (main, abaixo) quanto pelo
    app unificado com login (cofrya_app.py), que passa uma pasta isolada
    por usuário."""
    ...
    with verificacao:
        with st.form("verificar_backup"):
            ...
            if modo_arquivos == "Enviar agora pelo navegador":
                ...
                st.caption(
                    "Os arquivos ficam na pasta da sua conta no servidor. Baixe os resultados "
                    "para preservá-los fora desta hospedagem."
                )
            ...
            with st.expander("Parâmetros adicionais"):
                provedor_ambiente = st.selectbox(
                    "Ambiente de restauração", ["docker", "neon"],
                    index=1 if obter_valor("NEON_API_KEY") and obter_valor("NEON_PROJECT_ID") else 0,
                    key="provedor_ambiente",
